...
describe("user restore e2e", () => {
  ...
  describe("x-api-key", () => {
    it("should return 401 when not send x-api-key", async () => {
      const { status, body } = await request(app.getHttpServer()).patch(
        `${USER_RESTORE_URL}/${faker.string.uuid()}`,
      );
  ...
  describe("auth", () => {
    it("should return 401 when not auth", async () => {
      const { status, body } = await request(app.getHttpServer())
        .patch(`${USER_RESTORE_URL}/${faker.string.uuid()}`)
  ...
  describe("guard", () => {
    ...
    it("should return 404 when user is not deleted", async () => {
      const { status, body } = await request(app.getHttpServer())
        .patch(`${USER_RESTORE_URL}/${user._id}`)
        .set("x-api-key", xApiKey)
        .set("authorization", `Bearer ${adminAccessToken}`);
  ...
  describe("restore response", () => {
    it("should return 200 when restore user successful", async () => {
      const notExist = await userService.findOneById(userDelete._id);
      expect(notExist).toBeNull();

      const { status, body } = await request(app.getHttpServer())
        .patch(`${USER_RESTORE_URL}/${userDelete._id}`)
        .set("x-api-key", xApiKey)
        .set("authorization", `Bearer ${adminAccessToken}`);
      ...
      const exist = await userService.findOneById<UserDoc>(userDelete._id);

      expect(exist._id).toBe(userDelete._id);
