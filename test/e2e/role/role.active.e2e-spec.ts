...
describe("role active e2e", () => {
  ...
  let roleInActivePublicDoc: RoleDoc;
  ...
  beforeEach(async () => {
    ...
    // create userRole
    const [roleUser, roleAdmin, rolePublic, roleInActivePublic] = await Promise.all([
      createRoleUser(app, "user"),
      createRoleAdmin(app, "admin"),
      createRoleUser(app, "active public"),
      createInActiveRoleUser(app, "inactive public"),
    ]);
    rolePublicDoc = rolePublic;
    roleInActivePublicDoc = roleInActivePublic;
  ...
  describe(`PATCH ${UPDATE_URL}/role/active`, () => {
    describe("x-api-key", () => {
      it("should return 401 when not send x-api-key", async () => {
        const { status, body } = await request(app.getHttpServer()).patch(
          `${UPDATE_URL}/${roleInActivePublicDoc._id}/active`,
        );
    ...
    describe("auth", () => {
      it("should return 401 when not auth", async () => {
        const { status, body } = await request(app.getHttpServer())
          .patch(`${UPDATE_URL}/${roleInActivePublicDoc._id}/active`)
          .set("x-api-key", xApiKey);
    ...
    describe("get guard", () => {
      ...
      it("should return 404 when not not found role", async () => {
        const { status, body } = await request(app.getHttpServer())
          .patch(`${UPDATE_URL}/${faker.string.uuid()}/active`)
          .set("x-api-key", xApiKey)
          .set("Authorization", `Bearer ${adminAccessToken}`);
      ...
      it("should return 400 when role active yet", async () => {
        const { status, body } = await request(app.getHttpServer())
          .patch(`${UPDATE_URL}/${rolePublicDoc._id}/active`)
          .set("x-api-key", xApiKey)
          .set("Authorization", `Bearer ${adminAccessToken}`);
